...
const statuses: (CertificationStatus | "All")[] = [
  "All",
  "Published",
  "Draft",
  "Archived",
    ...
    <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 p-3.5 rounded-[var(--radius-lg)] border border-[var(--border-default)] bg-[var(--surface-card)]">
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div className="relative flex-1 sm:flex-none">
          ...
          >
            <option value="All" className="bg-[var(--surface-card)] text-[var(--text-primary)]">
              Year: All
            </option>
            {availableYears.map((year) => (
              <option key={year} value={year.toString()} className="bg-[var(--surface-card)] text-[var(--text-primary)]">
                {year}
              </option>
            ))}
          </select>
